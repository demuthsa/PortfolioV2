    ...
    >
      ...
      <div className="flex flex-col w-full max-w-6xl gap-8">
        {projects.map((project, index) => (
          <motion.div 
            key={index} 
            className="flex flex-col items-center w-full p-6 bg-transparent border-2 border-[#1c3d5a] rounded-lg shadow-lg hover:shadow-2xl md:flex-row transition-shadow duration-300 ease-in-out"
            variants={fadeInUp}
          ...
          >
            ...
            <div className="flex-1 w-full md:ml-6">
              <h3 className="mb-2 text-xl font-semibold text-center text-[#d0e1f9] md:text-left">{project.title}</h3>
              <div className="mb-4 text-center md:text-left">
                <h4 className="text-lg font-medium text-[#FF9E8A]">Made With</h4>
                <ul className="flex flex-wrap justify-center gap-2 mt-2 md:justify-start">
                  {project.madeWith.map((tech, techIndex) => (
                    <li key={techIndex} className="px-2 py-1 text-sm text-white bg-[#1c3d5a] rounded">
                      {tech}
                    </li>
                  ))}
                </ul>
              </div>
              <p className="mb-4 text-center text-[#d0e1f9] md:text-left">{project.description}</p>
              <div className="flex gap-4">
                {project.liveLink && (
                  <a 
                    ...
                    className="px-4 py-2 text-[#365465] border border-[#365465] rounded hover:text-[#FF9E8A] hover:border-[#FF9E8A]"
                  >
                    Live
                  </a>
                )}
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="flex items-center justify-center px-4 py-2 text-[#365465] border border-[#365465] rounded hover:text-[#FF9E8A] hover:border-[#FF9E8A]"
                >
                  <FaGithub size={20} />
                </a>
              </div>
